...
- **Handle `null` carefully.** The documentation of `StrCmpLogicalW` says nothing about `null`. On the test machine (Windows 10.0.26200), all three calls measured with a `null` argument, `(null, "a")`, `("a", null)`, and even `(null, null)`, returned `-2`, a value the documentation does not list. Swapping the arguments did not flip the sign and two `null`s did not compare equal, which breaks the contract a comparer must keep. Handle `null` before the call, as shown above.
