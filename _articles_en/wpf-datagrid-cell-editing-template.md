...
- When the editing control is written by hand in `CellEditingTemplate`, `Validation.ErrorTemplate` can be set on it as usual. The template renders on the adorner layer, so display depends on the visual tree providing one.
Columns whose editing element is generated by the framework, such as `DataGridTextColumn`, cannot use that approach and rely on `EditingElementStyle` instead (see [Why WPF Validation Errors Are Not Displayed, and Choosing Between IDataErrorInfo and INotifyDataErrorInfo](/articles/wpf-validation-error-not-displayed/)).
