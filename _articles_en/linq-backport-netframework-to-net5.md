...
**That guard only works in SDK-style projects, though.** `NET471_OR_GREATER` is a symbol defined automatically from `TargetFramework`, and a legacy-format project — one specifying `TargetFrameworkVersion` — does not define it.
The figure below builds the same source in both formats.

<figure class="article-figure">
  <img src="/images/articles/linq-backport-netframework-to-net5/linq-net5-project-format.svg" alt="A table of whether NET471_OR_GREATER is defined and what the build produces, per project format, for one and the same source. The SDK-style project defines the symbol, skips the polyfill, and builds. The legacy format does not define it, compiles the polyfill in, and fails with CS0121. Defining the symbol through DefineConstants makes the legacy format build." width="1047" height="170" loading="lazy">
  <figcaption>The polyfill placed in <code>namespace System.Linq</code> as the article does, with a call to <code>Append</code>, built in three project configurations. The <code>symbol</code> column is not inferred from whether the build succeeded: it comes from which branch of a <code>#warning</code> placed in the source fired.</figcaption>
</figure>
...
The third row of the table adds exactly that, and lands on the same result as the SDK-style project.
