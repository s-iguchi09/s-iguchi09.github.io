...
date: 2026-07-23
category: WPF
excerpt: "A custom RelayCommand leaves its button stuck because CanExecuteChanged is never raised. This covers delegating to CommandManager.RequerySuggested versus raising the event manually."
---
...
- Language: C# / XAML (samples assume nullable enabled and target-typed `new`; on C# 8 or earlier, use explicit types)
...
Forward the `add` / `remove` of `CanExecuteChanged` to `CommandManager.RequerySuggested`.
...
`InvalidateRequerySuggested` re-evaluates every bound command at once.

```csharp
// Call when a condition changed without a UI interaction
CommandManager.InvalidateRequerySuggested();
```

### Raise CanExecuteChanged manually

Keep `CanExecuteChanged` as a dedicated event and provide a method that raises it when re-evaluation is needed.
...
In the view model, call `RaiseCanExecuteChanged` right after updating a property that `CanExecute` depends on.
...
- **`RequerySuggested` holds handlers by weak reference:** `CommandManager.RequerySuggested` keeps registered handlers as weak references. In the delegating approach this is usually fine, because the command instance itself stays alive; but a custom implementation that registers a handler directly with `RequerySuggested` must keep a separate strong reference, or the handler is collected and re-evaluation stops.
...
- **Delegation re-evaluates every command:** `InvalidateRequerySuggested` re-queries `CanExecute` on all bound commands. Heavy work inside `CanExecute` makes frequent re-evaluation harm responsiveness, so keep `CanExecute` lightweight.
...
|---|---|---|---|
| Delegate to `RequerySuggested` | Minimal code; follows UI interactions automatically | Re-evaluates all commands; opaque trigger; weak-reference caveat | Executability tied mainly to UI interaction (focus, selection) |
...
| Call `InvalidateRequerySuggested` on demand | Re-evaluates at any moment while delegating | Cost of re-evaluating all commands; easy to forget | Reflecting UI-independent changes under the delegating approach |
