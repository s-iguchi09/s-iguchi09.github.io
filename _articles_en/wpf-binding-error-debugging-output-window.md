...
When `DataItem` names the element type of a collection instead of the expected view model, the cause is that the `DataContext` switches to each item inside a `DataTemplate` (see [Binding to the Parent DataContext from Inside a WPF DataTemplate](/articles/wpf-datatemplate-parent-datacontext-binding/)).
