...
**`DeleteButton` is present only on the row where `ThemeMode` is set and no implicit style overrides it.** That part is the clear button, and this confirms its name on `.NET 10`.
