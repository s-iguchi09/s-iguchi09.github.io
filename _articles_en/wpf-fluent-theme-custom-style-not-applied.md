...
- **A direct `Fluent.xaml` merge picks up the Windows theme setting at startup.** In measurement, merging `Fluent.xaml` on a machine set to dark mode loaded the dark dictionary internally. To pin the appearance, drop the manual merge and set `ThemeMode` to `Light` or `Dark`. Adding `ThemeMode` while keeping the manual merge leaves the control colors unpinned, because the manual merge wins.
