...
With no container, there is no binding to place on it.
Raising the item count leaves the number of realized containers at 31, so the proportion of items without a container grows with the collection.
...
After the ten `PageDown` operations, these counts show that selection was lost on one side and written to the data on the other as containers were realized, and that the two sides no longer agreed. The counts were read once, after all ten operations; the change after each scroll, and which items changed in which direction, were not measured.
