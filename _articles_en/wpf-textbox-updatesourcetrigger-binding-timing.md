...
<figure class="article-figure">
  ...
  <figcaption>In both rows <code>UserName</code> starts as <code>suzuki</code>, and <code>sato</code> is typed after moving focus into the input, through WPF's input processing (<code>InputManager</code>). The bottom box is typed first and the top box last, so focus stays in the top box. The right side displays the same <code>UserName</code> through a <code>OneWay</code> binding. With the default (top) the source is not updated because focus never leaves the box, while <code>PropertyChanged</code> (bottom) updates it immediately.</figcaption>
</figure>
...
- **Text under IME composition**: whether unconfirmed text during IME composition reaches the source with `PropertyChanged` was not measured in this article. To handle only committed values, use `LostFocus` or `Explicit` and decide the commit point yourself.
- **Validation timing**: `ValidationRules` are attached to the `Binding` and run around the source update according to their `ValidationStep` (default `RawProposedValue`), so they track the `UpdateSourceTrigger`. In contrast, `INotifyDataErrorInfo` validates on the ViewModel after the source is updated and reflects results through the `ErrorsChanged` notification, so with asynchronous validation the display timing may not coincide with the update trigger. With `LostFocus`, the source update (and `ValidationRules`) runs after leaving the field; with `PropertyChanged`, it runs per keystroke. A rule with `ValidatesOnTargetUpdated="True"` also runs on target updates, however, so it does not track the `UpdateSourceTrigger` alone. `INotifyDataErrorInfo` results likewise surface when `ErrorsChanged` is raised, which an implementation may do independently of the source update. Raising `ErrorsChanged` from a background thread still got the error onto the screen (see the table below). Isolating the cause when validation results never reach the screen is covered in [Why WPF Validation Errors Are Not Displayed, and Choosing Between IDataErrorInfo and INotifyDataErrorInfo](/articles/wpf-validation-error-not-displayed/).
