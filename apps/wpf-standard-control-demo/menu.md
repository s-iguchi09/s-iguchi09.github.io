...
- **Bind the shortcut with a `KeyBinding`, and set `InputGestureText` too.** With a `KeyBinding` on the window, a real <kbd>Ctrl</kbd>+<kbd>O</kbd> ran a command that has no key gesture of its own, but the item's `InputGestureText` stayed empty. `InputGestureText` only shows text; a command with a key gesture, such as `Copy`, fills it in by itself.
