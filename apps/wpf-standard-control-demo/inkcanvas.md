...
## What a drag does in each editing mode

An **InkCanvas** collects what is drawn in `Strokes`, a `StrokeCollection`. A drag with the real mouse added one stroke.

`EditingMode` decides what the mouse does; the default is `Ink`. A left-to-right drag drew a stroke with `Ink` and nothing with `None`. Across a line, `EraseByStroke` removed the whole line and `EraseByPoint` cut it in two. With `Select`, a click on the line selected it. `ActiveEditingMode`, which is read-only, is the mode in use: during a mouse drag it was `Ink`, the same as `EditingMode`. `EditingModeInverted` is the mode for the inverted end of a pen; the default is `EraseByStroke`. The mouse does not use it: while dragging with the default, `ActiveEditingMode` stayed `Ink`.

With `GestureOnly` and `InkAndGesture`, the same left-to-right drag was recognized as the gesture `Right`, and no stroke remained in either mode. Recognizing gestures needs a recognizer on the machine; `IsGestureRecognizerAvailable` was `True` on the measuring machine. To keep strokes that are not meant as gestures in the `InkAndGesture` mode, set `Cancel` to `True` in the `Gesture` handler, which kept the drag as a stroke; limiting the recognized gestures with `SetEnabledGestures` to a circle only also kept it.

## The pen, and the background behind it

`DefaultDrawingAttributes` is the pen for new strokes: by default black, about 2 × 2 (exactly 2.0031496062992127 in both directions), with an ellipse tip, and not a highlighter. Each stroke gets its own copy. After the pen's `Color` and `Width` were changed in place, as the demo app does, the first stroke stayed black and about 2 wide, and the next one was red and 10 wide. Changing the default pen affects only later strokes; to recolor a stroke, change that stroke's own `DrawingAttributes`.

The `Background` property's own default is `null`, but the default style sets it to the system window color (`SystemColors.WindowBrush`), white on the measuring machine, so the InkCanvas is not transparent. Even with `Background` set to `null` a drag still drew a stroke: the InkCanvas takes input over its whole area, so one without a background can be laid over a picture to annotate it.

Check the pen color against the background. The demo app starts with the pen color `AntiqueWhite` and the background `AliceBlue`, whose contrast ratio is only 1.09 : 1, so pick a darker pen color before drawing.

## Commands, saving, and undo

`ApplicationCommands` sent to the InkCanvas depend on the mode and the strokes. `SelectAll` could not execute in the `Ink` mode even with a stroke. In the `Select` mode it could not with no strokes, could with one, and selected it; after that, `Copy` could execute. Switch to `Select` before running `SelectAll`. `Undo` could not execute: the InkCanvas does not handle it, so keep your own history for undo.
...
The InkCanvas page of the demo app has one section with `EditingMode`, `EditingModeInverted`, `ActiveEditingMode`, the pen color and width, `Background`, and Copy, Cut, Paste, Select All, and Clear All buttons. The buttons send `ApplicationCommands` to the InkCanvas, and Clear All calls `Strokes.Clear()`. The pen color `AntiqueWhite` and the background `AliceBlue` are the second item of `Colors` and the first of `Brushes` in its combo boxes, and `EditingModeInverted` starts at `InkAndGesture`, the fourth value of the enumeration, rather than the default. The "Show Code" link under the section displays its XAML. The following XAML is part of that section (`InkCanvasUsageControl.xaml`), with the styles, the pen settings, the other buttons, and the surrounding GroupBoxes left out and the namespace declarations added. The InkCanvas is given a height of 200 in place of the demo app's layout. `markupextensions` is the prefix for the demo app's own markup extensions:
