...
The demo app has sections for `TabStripPlacement`, `ContentStringFormat`, and `SelectedContent` with `SelectedContentStringFormat`. The "Show Code" link under each section displays its XAML.
...
| --- | --- | --- |
| `TabStripPlacement` | `Top / Bottom / Left / Right` | The edge the tabs are on. The default is `Top`, but the demo app starts at `Left`, the first value of the `Dock` enumeration in its combo box. In a TabControl 300 × 150, `Top` and `Bottom` put the two tabs side by side, with the content area 294 wide below or above them. `Left` and `Right` stacked the tabs, each about 37 wide and 20 high, with their text still horizontal. The content area narrowed to 255.34. |
| `ContentStringFormat` | `string` | A format for the content of the pages. With the demo app's `"Format: {0}."`, the page of Tab1 showed "Format: Item1.". A TabItem's own `ContentStringFormat` is used when that tab is selected from the start: Tab2 with `"Own {0}"` showed "Own Item2". Switching to it from Tab1 did not apply it, though: the page showed "Format: Item2.", while `SelectedContentStringFormat` already read `"Own {0}"`. |
| `SelectedContent / SelectedContentStringFormat` | `object / string (ReadOnly)` | The content and the format of the selected page; the demo app shows both. With the first tab selected, they were `Item1`, the content before formatting, and `"Format: {0}."`. With no tab selected, `SelectedContent` was `null`. |

## XAML Example

The following XAML is the `SelectedContent` section of the demo app (`TabControlUsageControl.xaml`), with the styles and the surrounding GroupBoxes left out and the namespace declarations added:
...
- **Settings dialogs:** one page per group of settings.
- **Documents:** one tab per open document, generated from a collection with `ItemsSource`.
- **Side navigation:** tabs on the left for many categories.

## Tips and Best Practices

- **Check indexes before setting them.** An index past the last tab is ignored without an error, and a value below -1 throws `ArgumentException`.
- **Use either `Items` or `ItemsSource`.** Mixing them throws.
- **Set one `ContentStringFormat` on the TabControl,** or use templates, rather than different formats per TabItem. A tab's own format was not applied when switching to it.
- **Set `TabStripPlacement` explicitly** if you copy the demo's XAML; its combo box starts at `Left`.
