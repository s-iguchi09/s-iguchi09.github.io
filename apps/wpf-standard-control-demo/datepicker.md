...
## The value is a nullable date: bind it to DateTime?

The value of a **DatePicker** is `SelectedDate`, a `DateTime?` that is `null` when no date is chosen and binds two-way by default. The text box shows it as text, and typing a date and pressing Enter sets it; clearing the text box and pressing Enter set it to `null`. Bind it to a `DateTime?` property, for example for a due date that stays empty until the user picks one. Bound to a `DateTime` property, setting it to `null` produced a binding error: the source kept its old date, and `Validation.HasError` became `True` on the DatePicker.

`DisplayDate` is the month the calendar shows. A new DatePicker had today's date, and setting `SelectedDate` to 2026-04-15 moved `DisplayDate` to the same day. Setting `DisplayDate` afterward did not change `SelectedDate`.

The calendar popup is a `Calendar` control. `IsDropDownOpen` tells whether it is open and binds two-way by default. Setting it to `True` before the window was shown raised no exception, and the popup was open once the window appeared.

## The range limits only the calendar

`DisplayDateStart` and `DisplayDateEnd` are the first and last dates the calendar offers. With a range of 2026-04-10 to 2026-04-20, the day button for 04-05 in the popup was disabled. The range is not enforced anywhere else. Setting `SelectedDate` to 04-05 from code, setting `Text` to "4/5/2026", and typing "4/5/2026" followed by Enter all left the date at 04-05, without an exception or a `DateValidationError`. Validate the range in the ViewModel.

A `DisplayDateStart` later than the selected date does not take effect either: with 04-10 selected, `DisplayDateStart = 04-15` was pulled back to 04-10. So for a From and a To DatePicker, binding the To picker's `DisplayDateStart` to the From date has no effect while the To picker already holds an earlier date. Check the order of the two dates in the ViewModel.
...
Dates here were parsed and displayed with `xml:lang="en-US"`. `Text` is the date as text. Unlike `SelectedDate`, it does not bind two-way by default. Setting it is parsed immediately with the DatePicker's culture: "4/15/2026", "2026-04-15", and "April 15, 2026" all gave 2026-04-15, and `Text` then read "4/15/2026". A string that cannot be parsed ("15/4/2026" in en-US) cleared `SelectedDate` to `null`.

Typing into the text box behaves differently: "abc" followed by Enter raised `DateValidationError`, kept the previous date, and put the previous text back. Handle `DateValidationError` to tell the user that typed text was not accepted; otherwise the text box silently goes back to the previous date.

## Defaults that come from the style and the culture

Two defaults come from the default style and differ from the property metadata. `IsTodayHighlighted` is `False` in the metadata, but the default style sets `True`, so today is marked unless you turn it off. With `False`, the day button's display state changed from `Today` to `RegularDay` and its today marker became transparent. The button stayed enabled, so today can still be selected.

`SelectedDateFormat` has only two values, `Short` and `Long`. It is `Long` in the metadata, and the effective default is `Short`, set by the default style. The output of each value in several cultures, and how to show another format, are measured in [Customising the DatePicker Display Format in WPF](/articles/wpf-datepicker-custom-format/). The article changes the format through the DatePicker's text part, because `SelectedDate` itself holds a `DateTime`, not text.

`FirstDayOfWeek`, the day in the leftmost column, was passed on to the popup's `Calendar` with the same value. A new DatePicker takes it from the thread's current culture: it was `Sunday` for en-US and ja-JP, and `Monday` for de-DE and fr-FR. Set it explicitly only when the calendar should differ from the user's culture.
...
The DatePicker page of the demo app has a section for each of `SelectedDate`, `DisplayDate`, `DisplayDateStart` and `DisplayDateEnd`, `FirstDayOfWeek`, `IsDropDownOpen`, `IsTodayHighlighted`, `SelectedDateFormat`, and `Text`. The `DisplayDate` section binds one DatePicker's `DisplayDate` to another DatePicker's selection, one month ahead of today; the range section runs from last Monday to next Friday; the `IsDropDownOpen` check box follows the popup as well as controlling it; and the `Text` section binds `Text` one-way from a text box. The "Show Code" link under each section displays its XAML. The following XAML is the `DisplayDate Range (Start/End)` section (`DatePickerUsageControl.xaml`), with the styles and the surrounding GroupBoxes left out and the namespace declarations added. `DateTimeAdvancedConverter` is a converter in the demo app that computes last Monday and next Friday from today. `converters` is the prefix for the demo app's own converters:
