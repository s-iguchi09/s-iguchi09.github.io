...
1. **ブランチを用意する。まず必ず最新の `main` を取得してから開始する。**
   ...
   - **既に存在する場合**: `git switch <branch>` で切り替えてから、**共有状況に応じて更新方法を選ぶ**:
     - **未共有(未 push)の個人ブランチ**: `git rebase origin/main` で最新 `main` に追随する。
     - **push 済み・共有中のブランチ**: 履歴を書き換えない `git merge origin/main` を用いる(rebase は避ける)。
   ...
   > rebase を選んだ場合、既に push 済みのブランチへ反映するには `git push --force-with-lease` が必要になる
   > (通常の push は拒否される)。いずれの場合も、取得せず古い状態のまま進めない。競合が発生した場合は解消してから進める。
