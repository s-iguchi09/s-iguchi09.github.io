...
4. **指摘・失敗を解消する。** CI 失敗またはレビュー指摘があるたびに:
   ...
   - 修正の解釈に**重大な**曖昧さがある場合、大規模改修が必要な場合、または**破壊的・不可逆な内容判断**が
     必要な場合は、`AskUserQuestion` で確認する
     (オートモードで確認を残すのはこれらの場合に限る。マージ操作そのものは Phase 4 手順 8 の基準で
     自動判断し、これらに該当しなければ確認しない)。
...
**1 本も自然発見されなかった**(2026-09-09 JST に確認)。
...
dev.to の本文リンクには **`nofollow` が付かない**(`noopener noreferrer` のみ。2026-09-09 JST 実測)。
...
> 公開される。2026-09-09 JST に実際に踏んだ不具合で、LF へ統一して解消した。
...
> ⚠️ **dev.to の画像プロキシは SVG を変換できない。** 中身を SVG のまま
> `Content-Type: image/webp` で返すため、ブラウザは webp としてデコードを試みて失敗し、
> 図が壊れて alt だけが残る(2026-09-11 JST 実測。プロキシが返す実体のバイト数が元ファイルと
> 完全に一致し、先頭も `<svg` のままだった)。data URI での埋め込みは
> 「Invalid markdown detected!」で拒否された。
>
> そのため `devto-export.js` は SVG を参照する `<figure>` を、alt と figcaption を並べた
> 引用ブロックへ置き換える。図の内容は alt に書かれているので情報は失われず、
> 元記事へのリンクが 1 本増えるぶん転載の目的にも沿う。
> **PNG はプロキシが正しく変換するので触らない。**
