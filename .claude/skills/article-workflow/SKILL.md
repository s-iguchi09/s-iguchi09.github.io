...
4. **指摘・失敗を解消する。** CI 失敗またはレビュー指摘があるたびに:
   - 原因を診断して記事・設定を修正する。
   - **同じ系統の穴は、指摘された 1 件だけでなくまとめて直す。** 指摘を受けたら、その関数・
     モジュール全体を読み、**同じ失敗の仕方をする経路**(入力の欠落・型違い・形式違い・
     外部取得の失敗など)を列挙し、危険側に倒れるものは同じコミットで潰す。再レビューは
     1 時間に 1 回しか回せないため(手順 6)、**指摘 1 件の後出しがそのまま 60 分の待機になる**。
     PR #103 では同一系統の指摘が 4 回に分かれ、3 サイクル約 3 時間を無駄にした。
   - **見送ると決めたものは、その場で理由とともにユーザーへ書く。** 黙って抱えたまま
     後から「予想していた」と言わない。指摘されたときに反論できる形で残す。
...
リンクもしていない新記事が自然に拾われるかでしか測れない。ところが 2026-09-19 に公開した
判定記事は、このワークフローが例外なく 6.1 と 6.2 を提示したため日英とも登録され、翌日には
dev.to からもリンクされて**判定が無効になった**(2026-09-20 に API で確認)。**規律ではなく
仕組みで守る。**

保留する記事は `.claude/skills/article-workflow/index-hold.json` に書く。
  ...
  "holds": [
    {
      ...
      "since": "2026-10-04",
      "until": "2026-10-18",
      "reason": "手動登録をやめてよいかの判定。自然発見が機能しているかを測る"
    }
  ]
}
```

`slug` は引数と同じ規則(`[A-Za-z0-9][A-Za-z0-9._-]*`)で書く。**末尾に `/` を付けない。**
引数側は `foo/` を `foo` に正規化するため、台帳に `"foo/"` と書くと突き合わせに失敗して
**保留が黙って効かない**。外れた値は警告ではなく例外で止める(保留が効かないまま導線を出すより安全)。

`until` は必須。**過ぎると保留は自動で解除され、判定を促す出力に変わる**ので、消し忘れても
「保留中です」と嘘をつき続けることがない。dev.to の実状を台帳に持たない原則
(6.2)の唯一の例外がこれで、保留はユーザーの意図であり API からは読めないため台帳でしか表現できない。

`devto-export.js` は**引数に関わらず毎回 holds を全件見る**。保留中の記事については、

- 6.1 の登録 URL と 6.2 の追記行を**出さない**(指定が保留中だけなら何も出さずに終わる)
- 保留中の記事が既に dev.to からリンクされていれば、**判定が壊れている旨を出して exit 1**

事故は「別の記事を処理しているセッションが、ついでに判定記事へ 1 行足す」形で起きたため、
判定記事を引数に含めない実行でこそ警告が目に入る必要がある。

**保留中の記事に対して、このワークフローがやってはいけないこと:**

| やってはいけない | 理由 |
|---|---|
| Search Console の登録 URL を提示する | 押せば判定が無効になる |
| dev.to への追記行を提示する | 同上。dofollow の外部リンクは最も強い発見経路 |
| **他の記事の本文から保留中の記事へ内部リンクを張る** | 一覧以外の発見経路が増え、何が効いたか分からなくなる |
| 保留中の記事を `--export` で転載用に書き出す | 転載自体が外部リンクになる |

判定日が来たら、Search Console の URL 検査で日英 2 URL のインデックス状況を調べ、結果を
記録してから holds の該当要素を削除する。判定の読み方は次のとおり。

| 結果 | 読み | その後 |
|---|---|---|
| 日英とも拾われた | 自然発見が機能している | 手動登録と dev.to 追記をやめてよい |
| 英語版だけ拾われた | dev.to 経由のみ生きている | 日本語版の手動登録は続ける |
| どちらも拾われない | 発見経路は依然閉じている | 記事ごとに dev.to へ 1 行足す運用を続ける |

**判定が出るまで手動登録をやめない。** 埋もれた記事を後から取り戻すのに同じ手間がかかる。
...
# 保留中の記事(6.0)と、それが汚染されていないかもここに出る
...
「どの投稿に足すべきか」を確実に判断できる**。唯一の例外が 6.0 の `index-hold.json` で、
「あえてリンクしない」という意図だけは API から読めないため台帳に置く。
