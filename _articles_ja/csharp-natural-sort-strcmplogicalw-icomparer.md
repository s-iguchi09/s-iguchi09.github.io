...
- 言語: C# 9.0 以降（本記事の実行例はトップレベルステートメントを使用する。比較クラス自体は `is null` を使うため C# 7.0 以降で動作し、P/Invoke はどのバージョンでも可）
...
この「数字を数値として扱う」比較を提供するのが、Win32 API の `StrCmpLogicalW`（`shlwapi.dll`）である。
...
`StrCmpLogicalW` は比較子として必要な形で `null` を扱わないため（注意点を参照）、比較前に `null` を明示的に処理する。
...
- **`null` の扱いに注意する。** `StrCmpLogicalW` のドキュメントは `null` について何も書いていない。検証環境（Windows 10.0.26200）では、`null` を含めて測った 3 組の呼び出し、`(null, "a")`・`("a", null)`・`(null, null)` は、いずれもドキュメントに無い `-2` を返した。引数を入れ替えても符号が反転せず、`null` どうしも等しくならないため、比較子が守るべき契約が崩れる。上記実装のように呼び出し前に `null` を処理する。
...
見た目をエクスプローラーに近づけることを最優先するなら `StrCmpLogicalW`、移植性と制御性を優先するなら自前実装、という基準で選択する。
