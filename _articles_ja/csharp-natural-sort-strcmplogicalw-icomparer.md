...
- **`null` の扱いに注意する。** `StrCmpLogicalW` のドキュメントは `null` について何も書いていない。検証環境（Windows 10.0.26200）では、`null` を含む呼び出しはすべて、ドキュメントに無い `-2` を返した。`(null, "a")`、`("a", null)`、`(null, null)` のいずれもである。引数を入れ替えても符号が反転せず、`null` どうしも等しくならないため、比較子が守るべき契約が崩れる。上記実装のように呼び出し前に `null` を処理する。
