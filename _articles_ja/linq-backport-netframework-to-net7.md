...
- **.NET Core 3.0 以降**（本家 .NET 7 の `Order` を含む）は、全体を並べる場合に比較例外を `InvalidOperationException`（メッセージ: "Failed to compare two elements in the array."、内側例外が上記 `ArgumentException`）にラップする。net10.0 では `Order().ToList()` でラップされたが、全体を並べずに最小の要素を探す `Order().First()` では `ArgumentException` のままだった（表の最後の 2 行）。

つまり、本ポリフィルは .NET Framework 上で動く限り、全体を並べる場合に本家と例外型が一致しない。
