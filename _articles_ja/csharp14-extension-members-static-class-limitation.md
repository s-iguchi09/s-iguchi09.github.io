...
- `extension` ブロックは C# 14（.NET 10）以降でのみ使用できる。
  それ以前のバージョンでは `this` 引数を用いた従来の拡張メソッド構文に留まる。
- 静的拡張メンバーも、呼び出し側で定義元の名前空間を `using` する必要がある。
  忘れると `CS0117`（定義がありません）になり、拡張メンバーの解決失敗であることが分かりにくい。
- 静的クラスを拡張する `extension` ブロックには、ユーザー定義演算子を置けない（`CS9321`）。
  演算子は拡張対象の型を引数に取る必要があり、静的クラスはその型になれないためである。
- `Directory`（静的メソッド）と `DirectoryInfo`（インスタンスメンバー）では API の形が異なる。
  `Directory.Exists(path)` に相当するのは `directoryInfo.Exists` プロパティであり、引数の有無が違う。
...
- `Directory.Xxx(...)` の形で呼びたい場合は、`extension(Directory)` に静的メンバーを書く（解決策 A）。
- インスタンス形式で呼びたい場合は、`DirectoryInfo` など対応するインスタンス型に対して定義する（解決策 B）。
- C# 13 以前も対象に含める場合は、従来どおり静的ヘルパークラスにする（解決策 C）。
