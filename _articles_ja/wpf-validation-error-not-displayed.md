...
この段階を成立させるには、対象要素より上に `AdornerLayer` を提供する要素が要る。
`Window` のテンプレートを差し替えている場合に `AdornerDecorator` を含めるのが代表的な対処だが、前述のとおり `ScrollViewer` の内側であれば `ScrollContentPresenter` がレイヤーを提供するため、それだけで描画される。
