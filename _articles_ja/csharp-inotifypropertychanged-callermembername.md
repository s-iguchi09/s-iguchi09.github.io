...
date: 2026-07-16
category: C#
excerpt: "INotifyPropertyChanged の実装でプロパティ名を文字列で渡す冗長さを、CallerMemberName 属性で解消する方法を整理する。nameof との違い、SetProperty ヘルパー、依存プロパティ通知の限界まで実装例とともに解説する。"
---
...
`nameof` 演算子との使い分け、比較・代入・通知を 1 行にまとめる `SetProperty` ヘルパー、そして依存プロパティ通知における限界までを扱う。
...
値が変化した場合のみフィールドを更新して `PropertyChanged` を発行し、`true` を返す。
...
`CallerMemberName` と `nameof` は排他ではなく、自身の通知は `CallerMemberName`、依存プロパティの通知は `nameof` と、役割を分けて併用するのが実務上の定石である。
...
- 補完対象は呼び出し元のメンバー名のみである。ラムダ式やローカル関数の内部から呼び出すと、期待するプロパティ名にならない場合がある。
- 計算プロパティやインデクサーへの通知は自動化されないため、依存関係は自前で管理する。
...
| nameof 演算子 | コンパイル時検証・リネーム追従 | 呼び出しごとに名前の記述が必要 | 依存プロパティなど他プロパティへの通知 |
...
一方、計算プロパティへの依存通知だけは `CallerMemberName` で自動化できないため、`nameof` を併用して対象名を明示する。
