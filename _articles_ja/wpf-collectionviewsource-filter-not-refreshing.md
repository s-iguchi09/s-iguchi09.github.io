...
この表で測った操作のうち、既にビューが把握している項目を評価し直したのは `Refresh()` を呼んだときだけで、このとき 1,000 件すべてが評価された。
`Filter` の差し替えや `SortDescriptions` / `GroupDescriptions` の変更でもビューは組み直されるが、それらはこの表の対象外である。
...
`LiveFilteringProperties` が空のままでは、プロパティ変更による再判定が行われない。
ビューの初期構築・`Refresh`・項目追加時のフィルタ評価はそのまま働くため、フィルタ自体が無効になるわけではない。
...
- **`Refresh` は `Reset` を通知するため、`ItemsControl` は実現済みの項目コンテナを作り直す。** ビューに残り続ける項目のコンテナも再生成される。ライブフィルタが `Add` / `Remove` を通知するのはビューへの出入りが生じた項目についてだけで、ビューに残る項目のコンテナは再利用される。対象は実現済みのコンテナに限られ、仮想化が有効なら未生成の項目は影響を受けない（実現されるのは、表示範囲と、その前後に [`VirtualizingPanel.CacheLength`](https://learn.microsoft.com/dotnet/api/system.windows.controls.virtualizingpanel.cachelength) で決まるキャッシュ領域の分のコンテナである。その数は項目の件数ではなく、表示範囲の大きさ・項目の大きさ・キャッシュの設定で決まる）。いずれも `ListBox` での実測であり、コンテナの生成と再利用の実際はパネルの実装と仮想化の設定に依存する。項目コンテナに描画コストの高いテンプレートを載せている場合、この差が体感に出る。
