...
実行して確認したところ、次の 2 つの構成で `ConvertBack(false)` が発生した（後掲の表）。1 つは、それぞれ別の ViewModel の同名プロパティにバインドしたラジオボタンを 1 つずつ持つ 2 行に、同じ `GroupName` を与えた構成。もう 1 つは、バインドしたラジオボタン 1 つと、チェックを付けたバインドなしのラジオボタン 1 つを同じグループに置いた構成である。
