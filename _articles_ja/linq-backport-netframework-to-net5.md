---
layout: article-ja
title: "Append・Prepend・TakeLast・SkipLast を .NET Framework にバックポートする"
date: 2026-07-10
category: C#
...
.NET Framework 4.8 から .NET 5 に至る期間（.NET Core 2.0 〜 3.1）は、LINQ に大量のメソッドが加わった時期ではなく、内部実装の書き直しによる性能改善が中心の移行期であった。それでも `Append`・`Prepend`・`TakeLast`・`SkipLast` という 4 つの実用メソッドがこの間に追加されており、.NET Framework 環境ではこれらが使えないために回避コードを書き続けることになる。
