...
date: 2026-04-24
category: WPF
excerpt: "ListBox の仮想化有効時に選択状態が維持されない理由と、各アイテムの IsSelected で MVVM で安定させる方法を解説する。ItemContainerStyle のバインドだけでは選択が失われる条件と、SelectionChanged を併用した対処を実測で示す。"
...
これは 2 つの件数の差し引きで、`SelectedItems` は 155 件減り、`IsSelected` は 135 件増えた。
10 回の `PageDown` の後のこの件数から、コンテナが実体化されるにつれて、一方では選択が外れ、他方ではデータ側へ選択が書き込まれ、両者が食い違ったことが分かる。件数は 10 回の操作の後に 1 度読んだだけで、1 回ごとのスクロールでの変化や、どの項目がどちらに変わったかは測っていない。
...
すべてのコンテナが実体化される規模（表示範囲と、その前後の `VirtualizingPanel.CacheLength` で決まるキャッシュ領域に収まる件数。本記事の計測では高さ 600 の `ListBox` で 31 件が実体化された）であれば、これらの非対称性は表面化しない。
