---
layout: article-ja
title: "KeyValuePair・タプル版 ToDictionary を .NET Framework にバックポートする"
date: 2026-07-15
category: C#
...
.NET 8 は公開 `Enumerable` へ新しい演算子をほとんど追加しなかったが、`KeyValuePair` やタプルのシーケンスをセレクタなしで辞書化する `ToDictionary` のオーバーロードが実質的な追加である。.NET Framework 環境ではこれらが使えず、要素が既にキーと値の形をしていても `p => p.Key` のような恒等セレクタを書く必要がある。
