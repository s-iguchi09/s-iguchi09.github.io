...
- **†2**: `System.Index` / `System.Range`（.NET Core 3.0+ で追加された BCL 型）が必要。.NET Framework では NuGet パッケージ（`System.Runtime.CompilerServices.Unsafe` や `IndexRange` など）で追加参照するかポリフィルを自前で定義する必要がある。
