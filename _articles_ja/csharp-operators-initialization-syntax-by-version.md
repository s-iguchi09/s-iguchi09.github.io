...
.NET Framework 環境での C# 開発において、`??=`（ヌル合体割り当て演算子）を使用するとコンパイルエラーが発生する。
...
このエラーは、`??=` が C# 8.0 で導入された構文であり、.NET Framework では C# 8.0 の機能が部分的にしかサポートされないために発生する。

---

## 原因・背景

C# の言語バージョンと .NET のフレームワークバージョンは独立して管理されており、常に最新の C# 構文が利用できるわけではない。
特に .NET Framework は開発が凍結されており、C# 8.0 以降の一部の言語機能が利用できないか、制限付きのサポートにとどまる。

以下に、本記事で取り上げる演算子・構文と導入された C# バージョンの対応表を示す。

| 演算子 / 構文 | 導入バージョン | .NET Framework サポート |
| --- | --- | --- |
| `??`（ヌル合体） | C# 2.0 | ✅ 2.0 以降 |
| `as`（型キャスト） | C# 1.0 | ✅ 全バージョン |
| `is`（型チェック） | C# 1.0 | ✅ 全バージョン |
| `=>`（ラムダ / 式形式） | C# 3.0 / C# 6.0 | ✅ 3.5 以降 |
| `?.` `?[]`（ヌル条件） | C# 6.0 | ✅ 4.6 以降 |
| `nameof` | C# 6.0 | ✅ 4.6 以降 |
| `is` パターンマッチング | C# 7.0 | ✅ 4.7 以降 |
| `??=`（ヌル合体割り当て） | C# 8.0 | ⚠️ 部分サポート / 実質非推奨 |
| `!`（null 免除） | C# 8.0 | ⚠️ 部分サポート |
| `^`（末尾インデックス） | C# 8.0 | ⚠️ 部分サポート |
| `..`（範囲） | C# 8.0 | ⚠️ 部分サポート |
| `with`（with 式） | C# 9.0 | ❌ 非対応 |
| Target-typed `new` | C# 9.0 | ❌ 非対応 |
| `required` プロパティ | C# 11.0 | ❌ 非対応 |
| コレクション式 / プライマリコンストラクタ | C# 12.0 | ❌ 非対応 |
