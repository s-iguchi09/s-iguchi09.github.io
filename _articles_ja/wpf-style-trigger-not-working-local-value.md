...
`UserControl` の依存関係プロパティを内部から更新する場合も同じ差し替えが起き、利用側が設定したバインディングが外れる（[WPF の UserControl に定義した DependencyProperty へ内部からバインドできない原因と DataContext の設計](/ja/articles/wpf-usercontrol-dependencyproperty-binding-not-working/)）。
