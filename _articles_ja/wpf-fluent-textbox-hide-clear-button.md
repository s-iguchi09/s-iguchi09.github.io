...
Fluent テーマを適用した WPF の `TextBox` は、フォーカスが入るとテキスト右端にクリアボタン(×)を表示する。
...
`helper` は添付プロパティを定義したクラスの名前空間に対する XML 名前空間プレフィックスである。
...
public static partial class TextBoxHelper
{
    ...
    private static void OnSingleLineHideClearChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
    {
        ...
        if ((bool)e.NewValue)
        {
            // AcceptsReturn=True で .NET 10 の非表示トリガーを成立させる。
...
`helper` プレフィックスは方法 1 と同じく、添付プロパティを定義したクラスの名前空間を指す。
