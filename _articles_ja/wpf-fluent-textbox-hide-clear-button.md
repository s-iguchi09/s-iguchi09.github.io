...
- `ThemeMode` を設定した場合と `Fluent.xaml` を直接マージした場合の、どちらでもパーツが現れる。
- `BasedOn` を書かない暗黙スタイルを同じキーに置くと、どちらの経路でもパーツが消える。
- `BasedOn` で元のスタイルを引き継いだ暗黙スタイルでは、どちらの経路でも自前の `Setter` が効いたままパーツが残る。
...
方法 1 はパーツ名を直接参照する。実際に `.NET 9` の `ClearButton` から `.NET 10` の `DeleteButton` へ改名された実績があり、将来の更新で再び変わればパーツが見つからずクリアボタンが再表示される。ただしこの劣化は例外を伴わず、非表示化が効かなくなるだけである。
...
Fluent テーマの `TextBox` のクリアボタンを消すには、対象パーツ(`.NET 10` は `DeleteButton`、`.NET 9` は `ClearButton`)にローカル値で `Visibility=Collapsed` を設定する方法 1 が既定の選択肢になる。入力挙動を保ったまま確実に消せ、両バージョンに対応できる。
