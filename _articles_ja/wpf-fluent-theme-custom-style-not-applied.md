...
- `ThemeMode` を設定しない場合と `Light` を設定した場合とで、コントロールに供給されるテンプレートが変わる。
- アプリ側の暗黙スタイルが当たったかどうかは、この測定では `Style` プロパティで判別している。
- `BasedOn` を持たない暗黙スタイルを同じキーに置くと、Fluent のテンプレートが供給されなくなる。
- テンプレートの出どころは、そのテンプレートに固有の名前付きパーツで判別している。
