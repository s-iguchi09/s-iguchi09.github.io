...
## 8. 新規記事より既存記事の補強を優先する

記事は日英ペアで相当数が公開済みである。**新しいテーマを増やすことより、既存記事を実測で補強することを優先する。**

Search Console で「クロール済み - インデックス未登録」と判定された記事を実測で検証したところ、前提と図でバージョンが食い違う、計測していない性能差を断定している、推奨した実装が期待どおり動かない、といった誤りが見つかった(`guidelines.md` §12)。
同種の記事は他にも残っており、**新規記事を追加してもそれらは直らない**。
...
両方に該当する記事を最優先とする。
Search Console で「クロール済み - インデックス未登録」と判定されている記事があれば、それも優先する。
...
`article-ideas` で記事案を挙げるときは、新規テーマの案だけでなく **§8.1 で列挙した既存記事の補強案も含める**。
補強案には対象 slug と、何を実測で確かめるのかを書く。
