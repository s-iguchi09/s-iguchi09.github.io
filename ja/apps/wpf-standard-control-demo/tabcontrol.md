...
**TabControl** は `Selector` を継承しています。作ったばかりの TabControl の `SelectedIndex` は表示されるまで -1 で、表示されると最初のタブが選ばれました。タブは `Items` の `TabItem` でも `ItemsSource` からでも作れますが、両方は使えません。`Items` に TabItem があるときに `ItemsSource` を設定しても、`ItemsSource` があるときに `Items` に追加しても、`InvalidOperationException` になりました。

項目数以上のインデックスと一覧にない項目は、拒否されるのではなく無視されます。2 つのタブで `SelectedIndex = 5` としても例外はなく、0 のままでした。一覧にない TabItem を `SelectedItem` に設定しても、2 つ目のタブが選ばれたままでした。-1 未満は拒否され、`SelectedIndex = -2` では `ArgumentException` が発生しました。`SelectedIndex = -1` ではどのページも表示されませんでした。選択中のタブを取り除くと次のタブが選ばれ、3 つのうち 2 つ目を取り除くと、3 つ目（インデックス 1）が選ばれました。
...
デモアプリには、`TabStripPlacement`、`ContentStringFormat`、`SelectedContent` と `SelectedContentStringFormat` の欄があります。各欄の下の「Show Code」リンクで、その欄の XAML を表示できます。
...
| プロパティ | 設定値 | 説明 |
| --- | --- | --- |
| `TabStripPlacement` | `Top / Bottom / Left / Right` | タブを置く辺です。既定値は `Top` ですが、デモアプリはコンボボックスの `Dock` の最初の値の `Left` で始まります。300 × 150 の TabControl では、`Top` と `Bottom` で 2 つのタブが横に並び、その下か上に幅 294 の内容の領域がありました。`Left` と `Right` ではタブが縦に積まれ、それぞれ幅約 37、高さ約 20 で、文字は横書きのままでした。内容の領域の幅は 255.34 に狭まりました。 |
| `ContentStringFormat` | `string` | ページの内容の書式です。デモアプリと同じ `"Format: {0}."` では、Tab1 のページは「Format: Item1.」と表示されました。TabItem 自身の `ContentStringFormat` は、そのタブを最初から選んでいれば使われ、`"Own {0}"` の Tab2 は「Own Item2」と表示されました。ところが Tab1 から切り替えると適用されず、`SelectedContentStringFormat` はすでに `"Own {0}"` なのに、ページは「Format: Item2.」と表示されました。 |
| `SelectedContent / SelectedContentStringFormat` | `object / string (ReadOnly)` | 選択中のページの内容と書式で、デモアプリは両方を表示しています。最初のタブを選んでいるときは、書式を適用する前の `Item1` と `"Format: {0}."` でした。どのタブも選んでいないとき、`SelectedContent` は `null` でした。 |

## XAML 使用例

デモアプリ（`TabControlUsageControl.xaml`）の `SelectedContent` の欄から、スタイルと周囲の GroupBox を省き、名前空間の宣言を加えたものです。
...
- **設定のダイアログ** — 設定のまとまりごとに 1 ページにします。
- **文書** — 開いている文書ごとに 1 つのタブを、`ItemsSource` でコレクションから作ります。
- **横のナビゲーション** — 分類が多いときは、タブを左に並べます。

## ヒントとベストプラクティス

- **インデックスは設定する前に確かめる** — 項目数以上のインデックスはエラーにならず無視され、-1 未満は `ArgumentException` になります。
- **`Items` と `ItemsSource` はどちらか一方にする** — 混ぜると例外になります。
- **`ContentStringFormat` は TabItem ごとに変えず、TabControl に 1 つ設定するかテンプレートを使う** — タブ自身の書式は、切り替えたときに適用されませんでした。
- **デモアプリの XAML を写すなら `TabStripPlacement` を明示する** — コンボボックスは `Left` で始まります。
