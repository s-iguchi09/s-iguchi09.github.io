...
キーボードでのリサイズを止めたい場合は `Focusable="False"` を設定します。フォーカスを受け取らなくなり（`Focus()` が `False` を返しました）、矢印キーが届かなくなります。Esc キーでドラッグを取り消せます。ボタンを離す前に Esc キーを押すと、列幅はドラッグ前の値に戻りました。`Thumb` から継承した読み取り専用の `IsDragging` は、スプリッター上でマウスの左ボタンを押すと `True` になり、`CancelDrag()` を呼ぶと `False` に戻りました（Esc キーのあとで変わるかは測っていません）。ドラッグ中にスプリッターを強調表示するトリガーに使えます。

## 大きさの下限と、幅の保存

スプリッターが変える `ColumnDefinition` の `MinWidth`（行なら `RowDefinition` の `MinHeight`）を設定します。左へ 1,000 ドラッグすると左の列は 0 まで縮みましたが、その `ColumnDefinition` に `MinWidth="50"` を設定すると 50 で止まりました。
